...
{   
  ...
  for (i=folder.nChildren-1; i>=0; i--) {
    if (folder.children[i] != undefined){
		if (folder.isOpen) 
...
{ 
 ...
 // { 
	if (USEFRAMES)
	  docW = docW + "<a class='textlink' TARGET=\"basefrm\" "
	else
	  docW = docW + "<a class='textlink' TARGET=\"_top\" "
    if (this.hreference) 
  	{ 
    	docW = docW +  " href='" + this.hreference + "' "
    }   
    if (isTextLink) {
...
{ 
  ...
  if (USETEXTLINKS) 
    docW = docW + "<a class='textlink' " + fullLink + " id=\"itemTextLink"+this.id+"\">" + this.desc + "</a>"
  else 
    docW = docW + this.desc
...
function findObjFuzzy(id)
{
		var lastPound = id.lastIndexOf('#');
		var newTest = id.substr(0,lastPound);
		//alert("secondtest:" + newTest);
  var i=0;
  var nodeObj;
  var match = false;
   	var matchstring = "/" + newTest + "/g";
   	  		//alert(matchstring);
  if (typeof foldersTree.xID != "undefined") {
    nodeObj = indexOfEntries[i];
    for(i=0;i<nEntries;i++){
    	var xid = indexOfEntries[i].xID;
 
    	if (xid.match(newTest)){
    		//alert("we have a match");
    		//alert("xid:" + xid + " " + "orig:" + newTest);
    		id = xid;
    		i=nEntries+2;
    		match = true;
    	}
