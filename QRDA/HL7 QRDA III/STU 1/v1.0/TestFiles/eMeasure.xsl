...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:n1="urn:hl7-org:v3" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
	...
	<xsl:template match="n1:QualityMeasureDocument">
		<html>
			...
			<body>
				...
				<h1 class="h1center">
					<xsl:value-of select="$title"/>
					<xsl:if test="n1:subjectOf/n1:measureAttribute[n1:code[n1:originalText='NQF ID Number']]/n1:value/@extension">
						<xsl:text> (NQF </xsl:text>
						<xsl:value-of select="n1:subjectOf/n1:measureAttribute[n1:code[n1:originalText='NQF ID Number']]/n1:value/@extension"/>
						<xsl:text>)</xsl:text>
					</xsl:if>
	            </h1>
				-->
	...
	<xsl:template name="documentGeneral">
		...
		<tr>
			...
			<td width="30%">
				<xsl:variable name="emeasureid">
					<xsl:call-template name="show-name">
						<xsl:with-param name="name" select="n1:subjectOf/n1:measureAttribute[n1:code[n1:originalText='eMeasure Identifier']]/n1:value"/>
					</xsl:call-template>
				</xsl:variable>
				<xsl:choose>
					<xsl:when test="string-length($emeasureid)>0">
						<xsl:value-of select="$emeasureid"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:text></xsl:text>
					</xsl:otherwise>
		...
		<tr>
			...
			<td width="30%">
				<xsl:choose>
					<xsl:when test="n1:subjectOf/n1:measureAttribute[n1:code[n1:originalText='NQF ID Number']]/n1:value/@extension">
							<xsl:value-of select="n1:subjectOf/n1:measureAttribute[n1:code[n1:originalText='NQF ID Number']]/n1:value/@extension"/>
					</xsl:when>
					<xsl:otherwise>
		...
		<tr>
			...
			<td width="80%" colspan="3">
				<xsl:choose>
					<xsl:when test="n1:subjectOf/n1:measureAttribute/n1:code[@code='MSRSD']">
						<xsl:call-template name="show-value">
							<xsl:with-param name="value" select="n1:subjectOf/n1:measureAttribute[n1:code[@code='MSRSD']]/n1:value[@xsi:type='TS']"/>
						</xsl:call-template>
						<xsl:if test="n1:subjectOf/n1:measureAttribute[n1:code[@code='MSRED']]/n1:value[@xsi:type='TS']">
							<xsl:text> through </xsl:text>
							<xsl:call-template name="show-time">
								<xsl:with-param name="datetime" select="n1:subjectOf/n1:measureAttribute[n1:code[@code='MSRED']]/n1:value[@xsi:type='TS']"/>
							</xsl:call-template>
						</xsl:if>
					</xsl:when>
				</xsl:choose>
		...
		<!-- CMS ID -->
		<xsl:if test="n1:subjectOf/n1:measureAttribute/n1:code[n1:originalText='CMS ID Number']">
		<tr>
			<td width="20%" bgcolor="#656565" style="background-color:#656565">
				<span class="td_label">
					<xsl:text>CMS ID Number</xsl:text>
				</span>
			</td>
			<td width="80%" colspan="3">
				<xsl:choose>
					<xsl:when test="n1:subjectOf/n1:measureAttribute/n1:code[n1:originalText='CMS ID Number']">
						<xsl:call-template name="show-value">
							<xsl:with-param name="value" select="n1:subjectOf/n1:measureAttribute[n1:code[n1:originalText='CMS ID Number']]/n1:value"/>
						</xsl:call-template>
					</xsl:when>
					<xsl:otherwise>
	...
	<xsl:template name="subjectOf">
		...
		<!-- Transmission format -->
		<xsl:for-each select="n1:subjectOf/n1:measureAttribute[n1:code/n1:originalText='Transmission Format']">
			<xsl:call-template name="show-subjectOf"/>
		</xsl:for-each>
		<!-- Initial Patient Population -->
		<xsl:for-each select="n1:subjectOf/n1:measureAttribute[n1:code/n1:originalText='Initial Patient Population']">
			<xsl:call-template name="show-subjectOf"/>
		</xsl:for-each>
		<!-- Denominator -->
		<xsl:for-each select="n1:subjectOf/n1:measureAttribute[n1:code/n1:originalText='Denominator']">
			<xsl:call-template name="show-subjectOf"/>
		</xsl:for-each>		
		<!-- Denominator Exclusions -->
		<xsl:for-each select="n1:subjectOf/n1:measureAttribute[n1:code/n1:originalText='Denominator Exclusions']">
			<xsl:call-template name="show-subjectOf"/>
		</xsl:for-each>
		<!-- Numerator -->
		<xsl:for-each select="n1:subjectOf/n1:measureAttribute[n1:code/n1:originalText='Numerator']">
			<xsl:call-template name="show-subjectOf"/>
		</xsl:for-each>
		<!-- Numerator Exclusions -->
		<xsl:for-each select="n1:subjectOf/n1:measureAttribute[n1:code/n1:originalText='Numerator Exclusions']">
			<xsl:call-template name="show-subjectOf"/>
		</xsl:for-each>				
		<!-- Denominator Exceptions -->
		<xsl:for-each select="n1:subjectOf/n1:measureAttribute[n1:code/n1:originalText='Denominator Exceptions']">
			<xsl:call-template name="show-subjectOf"/>
		</xsl:for-each>	
		<!-- Measure Population -->
		<xsl:for-each select="n1:subjectOf/n1:measureAttribute[n1:code/n1:originalText='Measure Population']">
			<xsl:call-template name="show-subjectOf"/>
		</xsl:for-each>		
		<!-- Measure Observations -->
		<xsl:for-each select="n1:subjectOf/n1:measureAttribute[n1:code/n1:originalText='Measure Observations']">
			<xsl:call-template name="show-subjectOf"/>
		</xsl:for-each>
		<!-- Supplemental Data Elements -->
		<xsl:for-each select="n1:subjectOf/n1:measureAttribute[n1:code/n1:originalText='Supplemental Data Elements']">
			<xsl:call-template name="show-subjectOf"/>
		</xsl:for-each>
	</xsl:template>
